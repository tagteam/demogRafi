...
dk <- filter(idb,Land == "Denmark")
dk <- left_join(dk,select(x,aldersinterval,M),by = "aldersinterval")
summarize(dk,sum(M/1000*Risikotid))
summarize(x,tibble(sum(M/1000*R),sum(Dod)))

us <- filter(idb,Land == "United States")
us <- left_join(us,select(x,aldersinterval,M),by = "aldersinterval")
# indirekte standardisering
summarize(us,sum(M*Risikotid))
summarize(us,sum(M*V))
#+END_SRC
c(56373,1873021,1959083,3128620)
