...
mortalitetsrater bliver forklaret i Kapitel 1. Den summarisk
...
#+ATTR_LATEX: :options otherkeywords={ggplot,hent_data}, deletekeywords={list,dt,data,scale,D,R}
#+BEGIN_SRC R :results file graphics :file ./kapitel2/k2-aldersspecifik-mortalitet.pdf :exports none :session *R*
# R-koder der producerer figur 1
dod <- hent_data(register = "dod",la
nguage = "da",køn = c("M","K"),tid = "all",alder = "all_no_total") %>% rename(D = "INDHOLD")
...
mænd end for kvinder. Slutteligt beregner man ofte middellevetid inden
...
rater/. Disse er anderledes end raterne rater i Kapitel 1. I
det følgende betegner vi derfor alle raterne i Kapitel 1
som /summariske rater/. /Summarisk/ betyder
...
rater starter vi med at forklare begrænsingerne med de summariske rater
...
summariske rater er ret nemt at indse, som vi nu viser. En
...
viden, at danske kvinder lever længere end danske mænd. Forklaringen er som følger:
...
eksempel være 20 km/t. Mortalitetsraten betegner hastigheden
...
pyramiden, kan man tydeligt se forskellen mellem mænd og kvinder: Der
er flere ældre kvinder end ældre mænd. Pyramiden afspejler også
historiske begivenheder som anden verdenskrig og den faldende tendens i
...
altså for en given opdeling i aldersintervaller givet ved vektoren (en ordnet samling af $m$ tal):
...
Vi kigger på en befolkning i en kalenderperiode \([t_1,t_2]\) og inddeler
den i \(m\) aldersgrupper. Vi betegner med
...
For at sammenligne mortalitet i to befolkninger (
studiebefolkning \(A\) mod studiebefolkning \(B)\) kan man sammenligne de
...
tabel. Men med mange aldersgrupper er det nemmere at se forskellen i
...
#+ATTR_LATEX: :width 0.7\textwidth
#+CAPTION: Aldersspecifikke mortalitetsrater for hele den danske befolkning i 2022. Vi ser at
#+CAPTION: dødeligheden var højere for mænd i alle aldre undtagen mænd der er mindst 99 år gamle.
...
mortalitetsraten baseret på aldersfordelingen i den anden befolkning.
På den måde kan man sammenligne
...
danske mænd sammenlignet med danske kvinder, hvis aldersfordelingen havde
...
og indirekte standardisering som metoder.
...
\begin{align*}
V_x &= \frac{R_x}{R}
\intertext{hvor \(R\) betegner befolkningens totale risikotid i perioden ligesom i formel \eqref{eq:sum_risikotid}. Vi omskriver formlen for den aldersspecifikke
...
gennemsnitlige mortalitetsrater. Sidste linje indses at være rigtig, da for 
