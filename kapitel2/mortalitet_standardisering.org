...
referencebefolkning (S). Ideen er at se om studiebefolkning A har
højere mortalitet end studiebefolkning B hvis (hypotetisk) begge havde
sammen aldersfordeling som referencebefolkningen S. Vi fortolker den
#+begin_export latex
...
referencebefolkning \(S\).  Ideen er at vi nu kan direkte sammenligne
de standardiserede mortalitetsrater fra studiebefolkninger \(A\) og
\(B\) hvis reference befolkningen er den samme. Den hyppigste form af
...
reference befolkning \(S\).

#+ATTR_LATEX: :options otherkeywords={hent_data}, deletekeywords={c,D}
#+BEGIN_SRC R  :results output   :exports both  :session *R* :cache yes  
# Fordeling af risikotid i aldersintervaller
folk <- hent_data("FOLK1a","alder"=0:125,tid="2011K3",køn = c(1,2,"TOT"),language = "da")
af <- intervAlder(folk,breaks = c(0,25,50,75,125),by = "KØN",vars = "INDHOLD")
af <- rename(af,R = INDHOLD)
# Antal døde i aldersintervaller
D <- hent_data("DOD","alder"="all_no_total",tid="2011",køn = c("M","K"),language = "da")
dd <- intervAlder(D,breaks = c(0,25,50,75,125),by = "KØN",vars = "INDHOLD")
...
dødsfald i studiebefolkning \(A\) er givet ved
#+begin_export latex
\begin{equation*}
