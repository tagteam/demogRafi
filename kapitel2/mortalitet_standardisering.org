...
#+BEGIN_SRC R :results file graphics :file ./kapitel2/k2-summa-mortalitetsrater.pdf :exports none :session *R* :cache yes
# R-koder der producerer figur 1
dod=hent_data(register = "dod",køn = c("M","K"),tid = "all",alder = "all_no_total") %>% rename(D = "INDHOLD")
dod <- intervAlder(dod,breaks=c(-Inf,seq(5,85,5),Inf),right=FALSE,var="D",alder="ALDER",by=c("KØN","TID"),label_one = "0-4",label_last = "85")
N=hent_data(register = "befolk2",køn = c("1","2"),tid = 1974:2022,alder = "all_no_total") %>% rename(R = "INDHOLD")
N <- rename(N,aldersinterval = alder)
dat <- left_join(dod,N,by = c("KØN","TID","aldersinterval"))
dat <- mutate(dat,M = 1000*D/R)
dat <- mutate(dat,KØN = factor(KØN))
levels(dat$KØN) <- list("Kvinder" = "Women","Mænd" = "Men")
ggplot(dat,aes(TID,M,color = KØN))+geom_line()+geom_point()+
...
g + theme(axis.title.x = element_text(margin = margin(t = 20, r = 0, b = 0, l = 0)))
g + theme(plot.background = element_rect(fill = "gray88", colour = NA))
#+END_SRC

#+RESULTS[(2024-02-10 15:45:51) 5ae5a29c6ebaac9a16023c4a95d87888a72232d0]:
...
#+CAPTION: dødeligheden var højre for mænd for alle aldre undtagen aldersgruppe 99+
