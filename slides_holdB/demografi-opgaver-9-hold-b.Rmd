...
## Brug af **gt**-pakken {style="font-size: 50%;"}
...
res_table_opg_1 <- table_opg_1 %>%
  ...
  fmt_number(columns = c("Andel_2009", "Andel_2019"), decimals = 2, locale = "da") ## Andel angivet som kommatal med to decimaler; brug dansk formatering
res_table_opg_1
```
...
library(writexl) 
write_xlsx(res_table_opg_1, "table_opg_1.xlsx") ## filen gemmes i samme mappe som .Rmd-filen
```
...
table_opg_2 %>% 
  rename("Alder" = ALDER) %>%
  gt() %>%
  fmt_number(columns = c("Beskæftige_2009", "Risikotid_2009", "Risikotid_2019"), decimals = 0, locale = "da") %>%
  fmt_number(columns = c("Aldersfordeling_2009", "Aldersfordeling_2019", "Beskæftigelsesrate"), decimals = 2, locale = "da")
```

```{r}
table_opg_2 %>% summarize("Summarisk beskaeftigelsesrate" = sum(Beskæftige_2009)/sum(Risikotid_2009)*1000, "Standardiseret beskaeftigelsesrate" = sum(Beskæftigelsesrate * Aldersfordeling_2019)*1000) %>%
  gt() %>%
