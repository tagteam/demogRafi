...
Døden har forskellige årsager, som fortæller hvad der er sket lige
inden, for eksempel et trafikulykke, eller samenfatter et korter eller
længere sygdomsforløb forgående døden. I Danmark har man siden 1875
samlet data om dødsårsager og ejer i dag verdens største
...
døde i Danmark. Den demografiske analyse af dødsårsager er med hensyn
til formål og metode relateret til de andre fag i dette semester
(sygdomslære, epidemiologi og biostatistik). Vi bruger data fra
dødsårsagsregister til konstruktionen af årsagsspecifikke
dødelighedstavler (Afsnit ref:K3:aarsag) og beregningen af
restlivstidsrisiko for at dø af en bestemt årsag.

** Gruppering af dødsårsager

Dødsårsager i dødsårsagsregisteret er opdelt i 2 grupperinger, A og B
grupperinger, hvor B er en undergruppering af A
grupper. Statistikbankens register =DODA1= har antal døde fordelt på
26 A-grupper:
...
#+end_example
\normalsize
Vi ser at dødsfald på grund af kræft har med stor afstand været den
...
B-undergrupper. Hvor A-grupper er mere overordnede er B-grupper mere
...
\(x\)-år og \((x+k-1)\)-år gammel i perioden. Det giver følgende notation
...
#+name: fig:k3-ulykker
#+ATTR_LATEX: :width 0.9\textwidth
#+CAPTION: Udviklingen i raten af dødsulykker blandt dansker i alderen mellem 15 og 29. Kilde: statistikbankens =DOD1A= og =FOLK1A=.
...
periodemål lige som middellevetid, og beregnes med udgangspunkt i en
...
alderstrin beregnes antal dødsfald lige som i en almindelig
overlevelsestavle. Dødsårsagerne bliver opdelt i to grupper sådan at
det samlede antal dødsfald er summen af dødsfald på grund af årsag
\(Q\) og dødsfald på grund af andre årsager. Det samlede antal døde i
...
\end{equation} 
#+end_export
Fordi der er nu er to muligheder for at forlade tabelbefolkningen
hedder funktionen \(\ell_x\), der generer en årsagsspecifik
dødelighedstavle, dobbel-dekrementfunktion.

*** Det årsagsspecifikke restlivstidsrisiko 

Det samlede antal dødfald med årsag \(Q\) i tabelbefolkningen hvor
tabelpersonen er ældre end \(x\)-år er (for 1-års intervaller):

#+begin_export latex
\begin{align}
_{\infty}d^Q_x = &{}_1d_x^Q +{}_1d_{x+1}^Q + \cdots +{}_\infty d_{x^{max}}^Q 
\intertext{og restlivstidsrisikoen blandt $x$-årige for at dø af kræft beregnes som}
&\operatorname{LTR}_x^Q=\frac{({}_1d_x^Q +{}_1d_{x+1}^Q + \cdots +{}_\infty d_{x^{max}}^Q)}{\ell_x}.
\end{align}
#+end_export


*** Risiko for at dø af en bestemt årsag 

I tabelbefolkningen er det samlede antal dødsfald hvor årsagen var
\(Q\) mellem alder 0 til alder $x$ given ved (for 1-års intervaller):
#+begin_export latex
\begin{align}
...
\intertext{Tilsvarende er risiko for dødsfald med andre årsager inden alder \(x\):}
...
Vi beregner dødelighedstavlen for kræftdød bland mænd i 2020 i
Danmark. Vi henter folketal fra statistikbankens register =FOLK1a= og
antal døde på grund af kræft fra register =doda1=. Vi inddeler i 19
...
aldersspecifikke mortalitetsrater for mænd i Danmark i 2020 og andel
kræftdødsfald.
...
kraeftdodtavle_maend <- dodsaarsagtavle(data = x,
                                        ...
                                        alder = "aldersinterval",
                                        radix = 100000)
# fravælge kolonner som ikke er vigtig lige her og nu
...
Kolonnen \(\operatorname{LTR_Q}\) i aldersinterval fra \(x\) til
\(x+k-1\) angiver restlivstidsrisiko for kræftdød for en tabelperson
med alder \(x\) og \(\operatorname{risiko_Q}\) angiver risiko for at
en tabelperson dør på grund af kræft inden alder \(x+k\). Fra
dødelighedstavlen aflæser vi dermed, for eksempel, at sandsynligheden
for at en nyfødt dreng i 2020 dør på grund af kræft inden alder 74 er
10,1%, under antagelsen at de årsagsspecifikke mortalitetsrater for
mænd forbliver i al fremtid som de var i 2020. Vi ser også at
restlivstidsrisikoen for kræftdød er 27,8% for en 65-årig mænd, under
antagelsen, at de årsagsspecifikke mortalitetsrater for mænd forbliver
i al fremtid som de var i 2020.
